...
```bash
python3.6 setup.py extract_messages  # Extract messages in your pot file
```

Then you can either add new supported locale:

```bash
python3.6 setup.py init_catalog -l xx  # where XX is the locale you want to add. ex: fr
```

or update the existing ones:

```bash
python3.6 setup.py update_catalog
```
...
```bash
python3.6 setup.py compile_catalog  # Compile .mo files for each language
```
