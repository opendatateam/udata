        ...
        for prefix, directory in self.config.get("STATIC_DIRS", tuple()):
            if filename.startswith(prefix):
                ...
                if isfile(join(directory, real_filename)):
                    return send_static(
                        directory, real_filename, cache_timeout=cache_timeout
                    )
        abort(404)
    ...
    def route(self, rule, **options):
        def wrapper(func_or_cls):
            ...
            if isinstance(func_or_cls, types.FunctionType):
                ...
                self.add_url_rule(
                    rule, view_func=func_or_cls.as_view(endpoint), **options
                )
            return func_or_cls
...
def create_app(
    config="udata.settings.Defaults", override=None, init_logging=init_logging
):
