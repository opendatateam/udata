...
class UDataApi(Api):
    def __init__(self, app=None, **kwargs):
        ...
        self.authorizations = {
            "apikey": {"type": "apiKey", "in": "header", "name": HEADER_API_KEY}
        }
    ...
    def _apply_secure(self, func, permission=None):
        ...
        def wrapper(*args, **kwargs):
            if (
                ...
                and any(
                    ext in str(func) for ext in current_app.config["METHOD_BLOCKLIST"]
                )
            ):
                self.abort(
                    423,
    ...
    def validate(self, form_cls, obj=None):
        ...
        form = form_cls.from_json(
            request.json, obj=obj, instance=obj, meta={"csrf": False}
        )
