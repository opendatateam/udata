...
def print_graph(graph: Graph, node: Graph):
    for x, y, z in node:
        print_node(graph, y, z)

def print_node(graph: Graph, predicate, objects, level = 0, printed_ids = None):
    if printed_ids is None:
        printed_ids = set()

    nm = graph.namespace_manager
    indent = level * 4 * ' '
    inner_indent = (level + 1) * 4 * ' '

    print(f"{indent}<{predicate.identifier.n3(nm)}>")
    if isinstance(objects, Literal):
        if objects.toPython():
            print(f"{inner_indent}{objects.toPython()}", end='')
        else:
            print(f"**empty literal**")
        if (objects.language):
            print(f" (lang={objects.language})", end='')
        if (objects.datatype):
            print(f" (datatype={objects.datatype})", end='')
        print('')
    elif isinstance(objects, RdfResource):
        print(f"{inner_indent}Resource ID: {objects.identifier.toPython()}")
        if objects.identifier.toPython() not in printed_ids:
            printed_ids.add(objects.identifier.toPython())
            for x, y, z in objects:
                print_node(graph, y, z, level + 1, printed_ids=printed_ids)
        else:
            print(f"{inner_indent}Skipping resource because already printed.")
    else:
        print(objects.__class__)
        for x, y, z in objects:
            print_node(graph, y, z, level + 1, printed_ids=printed_ids)
    print(f"{indent}</{predicate.identifier.n3(nm)}>")
