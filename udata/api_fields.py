...
lazy_reference = api.model('LazyReference', {
...
def convert_db_to_field(key, field, info = {}):
    ...
    elif isinstance(field, mongo_fields.GenericReferenceField) or isinstance(field, mongoengine.fields.GenericLazyReferenceField):
...
def patch(obj, request): 
    ...
    for key, value in request.json.items():
        ...
        if field is not None and not field.readonly:
            ...
            elif isinstance(model_attribute, mongoengine.fields.GenericReferenceField) or isinstance(model_attribute, mongoengine.fields.GenericLazyReferenceField):
                value = wrap_primary_key(key, model_attribute, value['id'], document_type=db.resolve_model(value['class']))
...
def wrap_primary_key(field_name: str, foreign_field: mongoengine.fields.ReferenceField | mongoengine.fields.GenericReferenceField, value: str, document_type = None):
    '''
    ...
    if isinstance(foreign_field, mongoengine.fields.GenericReferenceField) or isinstance(foreign_field, mongoengine.fields.GenericLazyReferenceField):
    ...
    if isinstance(id_field, mongoengine.fields.ObjectIdField):
        ...
        raise ValueError(f"Unknown ID field type {id_field.__class__} for {document_type} (ID field name is {id_field_name}, value was {value})")
