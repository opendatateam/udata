...
class IndexingLifecycleTest(APITestCase):
    ...
    def test_producer_should_send_a_message_without_payload_if_not_indexable(
        self, mock_req
    ):
        ...
        url = (
            f"{search_service_url}{DatasetSearch.search_url}{str(fake_data.id)}/unindex"
        )
    ...
    def test_reindex_model(self, mock_session, mock_req):
        ...
        index_model(
            DatasetSearch, start=datetime.datetime(2022, 2, 20, 20, 2), reindex=True
        )
    ...
    def test_index_model_from_datetime(self, mock_req):
        DatasetFactory(
            id="61fd30cb29ea95c7bc0e1211",
            last_modified_internal=datetime.datetime(2020, 1, 1),
        )
        fake_data = DatasetFactory(
            id="61fd30cb29ea95c7bc0e1212",
            last_modified_internal=datetime.datetime(2022, 1, 1),
        )

        index_model(
            DatasetSearch, start=None, from_datetime=datetime.datetime(2023, 1, 1)
        )
        mock_req.assert_not_called()

        index_model(
            DatasetSearch, start=None, from_datetime=datetime.datetime(2021, 1, 1)
        )
