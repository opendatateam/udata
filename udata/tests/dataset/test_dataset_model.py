...
class DatasetModelTest:
    ...
    @pytest.mark.parametrize(
        "freq", ["fourTimesADay", "threeTimesADay", "semidaily", "daily"]
    )
    ...
    @pytest.mark.parametrize(
        "freq", ["fourTimesAWeek", "threeTimesAWeek", "semiweekly", "weekly"]
    )
    ...
    def test_next_update_quarterly(self):
        ...
        assert_equal_dates(
            dataset.next_update, datetime.utcnow() + timedelta(days=365 / 4)
        )
    ...
    def test_next_update_biennial(self):
        ...
        assert_equal_dates(
            dataset.next_update, datetime.utcnow() + timedelta(days=365 * 2)
        )

    def test_next_update_triennial(self):
        dataset = DatasetFactory(frequency="triennial")
        assert_equal_dates(
            dataset.next_update, datetime.utcnow() + timedelta(days=365 * 3)
        )

    def test_next_update_quinquennial(self):
        dataset = DatasetFactory(frequency="quinquennial")
        assert_equal_dates(
            dataset.next_update, datetime.utcnow() + timedelta(days=365 * 5)
        )
...
class ResourceSchemaTest:
    ...
    def test_resource_schema_objects_w_cache(self, rmock, mocker):
        ...
        assert (
            ResourceSchemaMockData.get_all_schemas_from_mock_data(
                with_datapackage_info=False
            )
            == ResourceSchema.all()
        )
    ...
    def test_resource_schema_validation(self, rmock):
        ...
        resource.schema = Schema(
            name="etalab/schema-irve-statique", version="1337.42.0"
        )
        resource.validate()
