...
class ReuseAPITest:
    ...
    def test_suggest_reuses_api(self, api):
        ...
        response = api.get(
            url_for("api.suggest_reuses"), qs={"q": "arealtestpref", "size": "5"}
        )
        assert200(response)
    ...
    def test_suggest_reuses_api_unicode(self, api):
        ...
        for i in range(4):
            ReuseFactory(
                title="testé-{0}".format(i) if i % 2 else faker.word(), visible=True
            )

        response = api.get(
            url_for("api.suggest_reuses"), qs={"q": "testé", "size": "5"}
        )
        assert200(response)
    ...
    def test_suggest_reuses_api_no_match(self, api):
        ...
        response = api.get(
            url_for("api.suggest_reuses"), qs={"q": "xxxxxx", "size": "5"}
        )
        assert200(response)
...
class ReuseBadgeAPITest:
    ...
    def test_delete_404(self, api):
        response = api.delete(
            url_for(
                "api.reuse_badge", reuse=self.reuse, badge_kind=str(self.factory().kind)
            )
        )
        assert404(response)
