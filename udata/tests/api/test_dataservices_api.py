...
class DataserviceAPITest(APITestCase):
    ...
    def test_dataservice_api_create(self):
        ...
        self.assertEqual(
            response.json["self_api_url"],
            "http://local.test/api/1/dataservices/updated-title/",
        )
        dataservice.reload()
    ...
    def test_dataservice_api_index(self):
        ...
        self.assertEqual(
            response.json["data"][0]["datasets"][0]["id"], str(dataset_b.id)
        )
        self.assertEqual(response.json["data"][1]["title"], "C")
        self.assertEqual(
            response.json["data"][1]["datasets"][0]["id"], str(dataset_a.id)
        )
        self.assertEqual(
            response.json["data"][1]["datasets"][1]["id"], str(dataset_b.id)
        )
        self.assertEqual(response.json["data"][2]["title"], "A")
        self.assertEqual(
            response.json["data"][2]["datasets"][0]["id"], str(dataset_a.id)
        )
        ...
        self.assertEqual(response.json["previous_page"], None)
        assert response.json["next_page"].endswith(
            url_for("api.dataservices", page_size=1, page=2)
        )
        self.assertEqual(response.json["page"], 1)
        self.assertEqual(response.json["total"], 3)
        ...
        response = self.get(
            url_for("api.dataservices", sort="title", dataset=str(dataset_a.id))
        )
        self.assert200(response)
        ...
        self.assertEqual(response.json["data"][0]["title"], "A")
        self.assertEqual(
            response.json["data"][0]["datasets"][0]["id"], str(dataset_a.id)
        )
        self.assertEqual(response.json["data"][1]["title"], "C")
        self.assertEqual(
            response.json["data"][1]["datasets"][0]["id"], str(dataset_a.id)
    ...
    def test_dataservice_api_index_with_wrong_dataset_id(self):
        response = self.get(
            url_for("api.dataservices", sort="title", dataset=str("xxx"))
        )
    ...
    def test_dataservice_api_create_with_custom_user_or_org(self):
        ...
        self.assert400(response)
        self.assertEqual(
            response.json["errors"]["organization"],
