...
class DatasetAPITest(APITestCase):
    ...
    def test_dataset_api_list_with_filters(self):
        ...
        geozone_dataset = DatasetFactory(
        ...
        response = self.get(
            url_for("api.datasets", temporal_coverage="2022-05-03-2022-05-04")
        )
        self.assert200(response)
        self.assertEqual(len(response.json["data"]), 1)
        self.assertEqual(
            response.json["data"][0]["id"], str(temporal_coverage_dataset.id)
        )
        ...
        self.assertEqual(len(response.json["data"]), 1)
        self.assertEqual(
            response.json["data"][0]["id"], str(schema_version2_dataset.id)
        )
    ...
    def test_dataset_new_resource_with_schema(self, rmock):
        ...
        rmock.get(
            "https://example.com/schemas", json=ResourceSchemaMockData.get_mock_data()
        )
...
class DatasetBadgeAPITest(APITestCase):
    ...
    def test_delete(self):
        ...
        response = self.delete(
            url_for(
                "api.dataset_badge", dataset=self.dataset, badge_kind=str(badge.kind)
            )
        )
        self.assertStatus(response, 204)
        self.dataset.reload()
...
class DatasetResourceAPITest(APITestCase):
    ...
    def test_reorder(self):
        ...
        response = self.put(
            url_for("api.resources", dataset=self.dataset), expected_order
        )
        self.assertStatus(response, 200)
        self.assertEqual(
            [str(r["id"]) for r in response.json],
            [str(r["id"]) for r in expected_order],
        )
        self.dataset.reload()
        self.assertEqual(
            [str(r.id) for r in self.dataset.resources],
            [str(r["id"]) for r in expected_order],
        )
    ...
    def test_update_404(self):
        ...
        response = self.put(
            url_for(
                "api.resource", dataset=self.dataset, rid=str(ResourceFactory().id)
            ),
            data,
        )
        self.assert404(response)
    ...
    def test_suggest_format_api_no_match(self):
        ...
        DatasetFactory(
            resources=[ResourceFactory(format=faker.word()) for _ in range(3)]
        )

        response = self.get(
            url_for("api.suggest_formats"), qs={"q": "test", "size": "5"}
        )
        self.assert200(response)
    ...
    def test_suggest_datasets_api(self):
        ...
        response = self.get(
            url_for("api.suggest_datasets"), qs={"q": "title-test", "size": "5"}
        )
        self.assert200(response)
    ...
    def test_suggest_datasets_acronym_api(self):
        ...
        response = self.get(
            url_for("api.suggest_datasets"), qs={"q": "acronym-test", "size": "5"}
        )
        self.assert200(response)
    ...
    def test_suggest_datasets_api_unicode(self):
        ...
        response = self.get(
            url_for("api.suggest_datasets"), qs={"q": "title-testé", "size": "5"}
        )
        self.assert200(response)
    ...
    def test_suggest_datasets_api_no_match(self):
        ...
        response = self.get(
            url_for("api.suggest_datasets"), qs={"q": "xxxxxx", "size": "5"}
        )
        self.assert200(response)
        self.assertEqual(len(response.json), 0)
...
class CommunityResourceAPITest(APITestCase):
    ...
    def test_resources_api_list(self):
        ...
        self.assertEqual(
            len(set(res["id"] for res in resources)), len(community_resources)
        )
    ...
    def test_community_resource_api_get_from_string_id(self):
        ...
        response = self.get(
            url_for("api.community_resource", community=str(community_resource.id))
        )
        self.assert200(response)
        data = json.loads(response.data)
        self.assertEqual(data["id"], str(community_resource.id))
    ...
    def test_community_resource_api_update(self):
        ...
        data["description"] = "new description"
        response = self.put(
            url_for("api.community_resource", community=community_resource), data
        )
        self.assert200(response)
        self.assertEqual(CommunityResource.objects.count(), 1)
        self.assertEqual(
            CommunityResource.objects.first().description, "new description"
        )
...
class DatasetSchemasAPITest:
    ...
    def test_dataset_schemas_api_list(self, api, rmock, app):
        ...
        assert (
            response.json
            == ResourceSchemaMockData.get_expected_assignable_schemas_from_mock_data()
        )
...
class HarvestMetadataAPITest:
    ...
    dataset_harvest_fields["dynamic_field"] = fields.String(
        description="", allow_null=True
    )
    resource_harvest_fields["dynamic_field"] = fields.String(
        description="", allow_null=True
    )
    ...
    def test_dataset_with_harvest_computed_dates(self, api):
        ...
        dataset = DatasetFactory(
            resources=[ResourceFactory(harvest=resource_harvest_metadata)]
        )
        ...
        assert (
            response.json["resources"][0]["last_modified"]
