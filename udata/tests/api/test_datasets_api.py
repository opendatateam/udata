...
class DatasetAPITest(APITestCase):
    ...
    def test_dataset_new_resource_with_schema(self):
        ...
        response = self.put(url_for('api.dataset', dataset=dataset), data)
        self.assert400(response)
        assert response.json['errors']['resources'][0]['schema']['url'][0] == _('Invalid URL')
...
class DatasetSchemasAPITest:
    ...
    def test_dataset_schemas_api_list(self, api, rmock, app):
        ...
        rmock.get('https://example.com/schemas', json=self.CATALOG_SCHEMAS)
        response = api.get(url_for('api.schemas'))

        assert200(response)
        assert response.json == self.CATALOG_SCHEMAS['schemas']
    ...
    @pytest.mark.options(SCHEMA_CATALOG_URL='https://example.com/schemas')
    def test_dataset_schemas_api_list_error_w_cache(self, api, rmock, mocker, app):
        ...
        mocker.patch.object(cache, 'get', return_value=self.CATALOG_SCHEMAS['schemas'])

        # Endpoint becomes unavailable, cache should be used.
        rmock.get('https://example.com/schemas', status_code=500)
