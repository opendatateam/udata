...
class TopicsAPITest(APITestCase):
    ...
    def test_topic_api_get(self):
        ...
        data = response.json
        assert all(
            str(d.id) in (_d["id"] for _d in data["data"]) for d in topic.datasets
        )
...
class TopicDatasetsAPITest(APITestCase):
    ...
    def test_add_datasets_double(self):
        ...
        response = self.post(
            url_for("apiv2.topic_datasets", topic=topic),
        ...
        response = self.post(
            url_for("apiv2.topic_datasets", topic=topic), [{"id": dataset.id}]
        )
        assert response.status_code == 201
        topic.reload()
        assert len(topic.datasets) == 4
    ...
    def test_add_datasets_wrong_payload(self):
        ...
        topic = TopicFactory(owner=owner)
        response = self.post(
            url_for("apiv2.topic_datasets", topic=topic), [{"id": "xxx"}]
        )
        assert response.status_code == 400
        response = self.post(
            url_for("apiv2.topic_datasets", topic=topic), [{"nain": "portekoi"}]
        )
        assert response.status_code == 400
        response = self.post(
            url_for("apiv2.topic_datasets", topic=topic), {"non": "mais"}
        )
        assert response.status_code == 400
...
class TopicDatasetAPITest(APITestCase):
    def test_delete_dataset(self):
        ...
        response = self.delete(
            url_for("apiv2.topic_dataset", topic=topic, dataset=dataset)
        )
...
class TopicReusesAPITest(APITestCase):
    ...
    def test_add_reuses_double(self):
        ...
        reuse = ReuseFactory()
        response = self.post(
            url_for("apiv2.topic_reuses", topic=topic),
        ...
        response = self.post(
            url_for("apiv2.topic_reuses", topic=topic), [{"id": reuse.id}]
        )
        assert response.status_code == 201
        topic.reload()
        assert len(topic.reuses) == 4
    ...
    def test_add_reuses_wrong_payload(self):
        ...
        topic = TopicFactory(owner=owner)
        response = self.post(
            url_for("apiv2.topic_reuses", topic=topic), [{"id": "xxx"}]
        )
        assert response.status_code == 400
        response = self.post(
            url_for("apiv2.topic_reuses", topic=topic), [{"nain": "portekoi"}]
        )
        assert response.status_code == 400
        response = self.post(
            url_for("apiv2.topic_reuses", topic=topic), {"non": "mais"}
        )
        assert response.status_code == 400
