...
class DcatBackend(BaseBackend):
    ...
    def walk_graph(
        self, url: str, fmt: str
    ) -> Generator[tuple[int, Graph], None, None]:
        """
    ...
    def process_one_datasets_page(self, page_number: int, page: Graph):
        for node in page.subjects(RDF.type, DCAT.Dataset):
            ...
            self.process_dataset(
                remote_id, page_number=page_number, page=page, node=node
            )
    ...
    def inner_process_dataset(
        self, item: HarvestItem, page_number: int, page: Graph, node
    ):
    ...
    def walk_graph(
        ...
        while tree:
            ...
            tree = ET.fromstring(
                self.post(
                    url,
                    data=body.format(start=start, schema=self.DCAT_SCHEMA),
                    headers=headers,
                ).content
            )
    ...
    def walk_graph(
        ...
        while tree:
            ...
            search_results = tree_before_transform.find(
                "csw:SearchResults", {"csw": CSW_NAMESPACE}
            )
            if search_results is None:
                log.error(f"No search results found for {url} on page {page_number}")
                break
            ...
            response = self.post(
                url,
                data=body.format(start=start, schema=self.ISO_SCHEMA),
                headers=headers,
            )
            response.raise_for_status()
