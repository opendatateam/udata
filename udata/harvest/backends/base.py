...
class BaseBackend(object):
    ...
    def harvest(self):
        ...
        self.job = factory(
            status="initialized", started=datetime.utcnow(), source=self.source
        )
        ...
        try:
            ...
            error = HarvestError(
                message=safe_unicode(e), details=traceback.format_exc()
            )
            self.job.errors.append(error)
        finally:
            self.end_job()
    ...
    def process_dataset(self, remote_id: str, **kwargs):
        ...
        item = HarvestItem(
            status="started", started=datetime.utcnow(), remote_id=remote_id
        )
        ...
        try:
            ...
            dataset.harvest = self.update_dataset_harvest_info(
                dataset.harvest, item.remote_id
            )
            ...
            item.status = "failed"
            log.exception(
                f"Error while processing {item.remote_id} : {safe_unicode(e)}"
            )
    ...
    def update_dataset_harvest_info(
        self, harvest: HarvestDatasetMetadata | None, remote_id: int
    ):
        if not harvest:
    ...
    def autoarchive(self):
        ...
        for dataset in local_items_not_on_remote:
            if not dataset.harvest.archived_at:
                archive_harvested_dataset(
                    dataset, reason="not-on-remote", dryrun=self.dryrun
                )
