...
def mock_csw_pagination(rmock, path, pattern):
    ...
    def callback(request, context):
        ...
        with open(
            os.path.join(CSW_DCAT_FILES_DIR, pattern.format(page))
        ) as cswdcatfile:
...
class DcatBackendTest:
    ...
    def test_harvest_dataservices(self, rmock):
        ...
        assert (
            dataservices[0].base_api_url
            == "https://data.paris2024.org/api/explore/v2.1/"
        )
        assert (
    ...
    def test_harvest_big_catalog(self, rmock):
        ...
        resources_by_title = {
            resource["title"]: resource for resource in datasets["1"].resources
        }
        ...
        assert (
            get_from_json(
                current_app.config.get("HARVEST_GRAPHS_S3_BUCKET"), job.data["filename"]
            )
            is not None
        )
    ...
    @pytest.mark.options(
        SCHEMA_CATALOG_URL="https://example.com/schemas", HARVEST_MAX_ITEMS=2
    )
    ...
    def test_simple_nested_attributes(self, rmock):
        ...
        source = HarvestSourceFactory(
            backend="dcat", url=url, organization=OrganizationFactory()
        )
    ...
    def test_hydra_partial_collection_view_pagination(self, rmock):
        url = mock_pagination(
            rmock, "catalog.jsonld", "partial-collection-{page}.jsonld"
        )
        org = OrganizationFactory()
        source = HarvestSourceFactory(backend="dcat", url=url, organization=org)
    ...
    def test_xml_catalog(self, rmock):
        ...
        resource_1 = next(
            res for res in dataset.resources if res.title == "Resource 1-1"
        )
        ...
        resource_2 = next(
            res for res in dataset.resources if res.title == "Resource 1-2"
        )
        ...
        resource_3 = next(
            res for res in dataset.resources if res.title == "Resource 1-3"
        )
...
class CswDcatBackendTest:
    def test_geonetworkv4(self, rmock):
        url = mock_csw_pagination(
            rmock, "geonetwork/srv/eng/csw.rdf", "geonetworkv4-page-{}.xml"
        )
        org = OrganizationFactory()
        ...
        assert (
            dataset.title
            == "Localisation des accidents de la circulation routière en 2017"
        )
        assert (
            dataset.description
            == "Accidents corporels de la circulation en Hauts de France (2017)"
        )
...
class CswIso19139DcatBackendTest:
    def test_geo2france(self, rmock):
        with open(os.path.join(CSW_DCAT_FILES_DIR, "XSLT.xml"), "r") as f:
            xslt = f.read()
        url = mock_csw_pagination(
            rmock, "geonetwork/srv/eng/csw.rdf", "geonetwork-iso-page-{}.xml"
        )
        rmock.get(CswIso19139DcatBackend.XSL_URL, text=xslt)
        org = OrganizationFactory()
        source = HarvestSourceFactory(
            backend="csw-iso-19139", url=url, organization=org
        )
        ...
        resource = dataset.resources[0]
        assert (
            resource.title == "Téléchargement direct du lot et des documents associés"
        )
        assert (
            resource.url
