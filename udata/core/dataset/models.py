    ...
    def clean(self, **kwargs):
        ...
        if not self.url and not self.name:
            ...
            if self.version:
                raise FieldValidationError(
                    _(
                        "A schema must contains a name or an URL when a version is provided."
                    ),
                    field="version",
                )
        ...
        if self.version:
            allowed_versions = list(
                map(
                    lambda version: version["version_name"], existing_schema["versions"]
                )
            )
...
class DatasetQuerySet(OwnedQuerySet):
    ...
    def hidden(self):
        return self(
            db.Q(private=True) | db.Q(deleted__ne=None) | db.Q(archived__ne=None)
        )
...
class Dataset(WithMetrics, BadgeMixin, Owned, db.Document):
    ...
    def url_for(self, *args, **kwargs):
        return endpoint_for(
            "datasets.show", "api.dataset", dataset=self, *args, **kwargs
        )
    ...
    def frequency_label(self):
        return UPDATE_FREQUENCIES.get(
            self.frequency or "unknown", UPDATE_FREQUENCIES["unknown"]
        )
    ...
    def quality(self):
        ...
        result["dataset_description_quality"] = (
            ...
            > current_app.config.get("QUALITY_DESCRIPTION_LENGTH")
            else False
        )
    ...
    def add_resource(self, resource):
        ...
        self.update(
            __raw__={
                "$push": {"resources": {"$each": [resource.to_mongo()], "$position": 0}}
            }
        )
        self.reload()
        self.on_resource_added.send(
            self.__class__, document=self, resource_id=resource.id
        )
    ...
    def json_ld(self):
        result = {
            ...
            "dateModified": self.last_modified.isoformat(),
            "url": endpoint_for(
                "datasets.show", "api.dataset", dataset=self, _external=True
            ),
            "name": self.title,
    ...
    def count_discussions(self):
        ...
        self.metrics["discussions"] = Discussion.objects(
            subject=self, closed=None
        ).count()
        self.save()
