    ...
    def parse_filters(datasets, args):
        ...
        if args.get("license"):
            datasets = datasets.filter(
                license__in=License.objects.filter(id=args["license"])
            )
        if args.get("geozone"):
        ...
        if args.get("schema_version"):
            datasets = datasets.filter(
                resources__schema__version=args["schema_version"]
            )
        if args.get("topic"):
...
@ns.route(
    "/<dataset:dataset>/rdf.<format>", endpoint="dataset_rdf_format", doc=common_doc
)
@api.response(404, "Dataset not found")
@api.response(410, "Dataset has been deleted")
...
@ns.route(
    "/<dataset:dataset>/upload/community/", endpoint="upload_new_community_resource"
)
@api.doc(**common_doc)
...
@ns.route(
    "/<dataset:dataset>/resources/<uuid:rid>/", endpoint="resource", doc=common_doc
)
@api.param("rid", "The resource unique identifier")
...
class CommunityResourcesAPI(API):
    ...
    def get(self):
        ...
        return community_resources.order_by(args["sort"]).paginate(
...
@ns.route(
    "/community_resources/<crid:community>/",
...
class MimesSuggestAPI(API):
    ...
    def get(self):
        ...
        results = [
            {"text": i}
            for i in current_app.config["ALLOWED_RESOURCES_MIMES"]
            if args["q"] in i
        ]
        results = results[: args["size"]]
        return sorted(results, key=lambda o: len(o["text"]))
