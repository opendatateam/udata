...
class SchemaForm(ModelForm):
    ...
    def validate(self, extra_validators=None):
        ...
        try:
            Schema(
                url=self.url.data, name=self.name.data, version=self.version.data
            ).clean(check_schema_in_catalog=True)
...
def validate_contact_point(form, field):
    ...
    if field.data:
        if form.organization.data:
            ...
            contact_point = ContactPoint.objects(
                id=field.data.id, owner=form.owner.data
            ).first()
...
class DatasetForm(ModelForm):
    ...
    description = fields.MarkdownField(
        ...
        description=_(
            "The details about the dataset " "(collection process, specifics...)."
        ),
    )
