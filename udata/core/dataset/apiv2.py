...
dataset_fields = apiv2.model(
    "Dataset",
    {
        ...
        "contact_point": fields.Nested(
            ...
            allow_null=True,
            description="The dataset's contact point",
        ),
    },
...
class ResourcesAPI(API):
    ...
    def get(self, dataset):
        ...
        list_resources_url = url_for(
            "apiv2.resources", dataset=dataset.id, _external=True
        )
        ...
        if page > 1:
            ...
            offset = 0
        paginated_result = res[
            offset : (page_size + offset if page_size is not None else None)
        ]
...
@ns.route(
    "/<dataset:dataset>/resources/<uuid:rid>/extras/",
    endpoint="resource_extras",
    doc=common_doc,
)
