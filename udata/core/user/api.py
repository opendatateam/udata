...
class MyOrgCommunityResourcesAPI(API):
    ...
    def get(self):
        ...
        q = filter_parser.parse_args().get("q")
        owners = list(current_user.organizations) + [current_user.id]
        community_resources = CommunityResource.objects.owned_by(*owners).order_by(
            "-last_modified"
        )
        if q:
...
class UserListAPI(API):
    ...
    def get(self):
        ...
        if args["q"]:
            ...
            if args["sort"]:
                return search_users.order_by(args["sort"]).paginate(
        ...
        if args["sort"]:
            return users.order_by(args["sort"]).paginate(
                args["page"], args["page_size"]
            )
...
class UserAPI(API):
    ...
    def delete(self, user):
        ...
        if user == current_user._get_current_object():
            api.abort(
                403,
                "You cannot delete yourself with this API. "
