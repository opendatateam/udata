...
class Site(WithMetrics, db.Document):
    ...
    def count_users(self):
        ...
        self.metrics["users"] = User.objects(
            confirmed_at__ne=None, deleted=None
        ).count()
        self.save()
    ...
    def count_max_dataset_followers(self):
        ...
        self.metrics["max_dataset_followers"] = (
            dataset.metrics["followers"] if dataset else 0
        )
        self.save()

    def count_max_dataset_reuses(self):
        dataset = (
            Dataset.objects(metrics__reuses__gt=0)
            .visible()
    ...
    def count_max_reuse_datasets(self):
        reuse = (
            Reuse.objects(metrics__datasets__gt=0)
            .visible()
    ...
    def count_max_reuse_followers(self):
        reuse = (
            Reuse.objects(metrics__followers__gt=0)
            .visible()
            .order_by("-metrics.followers")
            .first()
        )
