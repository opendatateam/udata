...
class SuggestZonesAPI(API):
    ...
    def get(self):
        ...
        geozones = GeoZone.objects(
            Q(name__icontains=args["q"]) | Q(code__icontains=args["q"])
        )

        # We're manually sorting based on zone level int (cause we don't have the int value directly in mongo document)
        level_id_to_int_level = {
            level.id: level.admin_level for level in GeoLevel.objects()
        }
...
dataset_parser.add_argument(
    "size",
    type=int,
    help="The amount of datasets to fetch",
    location="args",
    default=25,
)
...
class ZonesAPI(API):
    @api.doc(
        "spatial_zones", params={"ids": "A zone identifiers list (comma separated)"}
    )
...
class ZoneDatasetsAPI(API):
    ...
    def get(self, id):
        ...
        datasets = []
        datasets += list(
            Dataset.objects.visible().filter(spatial__zones=zone).limit(args["size"])
        )
        return datasets
