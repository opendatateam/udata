...
DEFAULT_LEVELS_FILE = (
    "https://www.data.gouv.fr/fr/datasets/r/e0206442-78b3-4a00-b71c-c065d20561c8"
)
...
def load_zones(col, json_geozones):
    ...
    for _, geozone in enumerate(json_geozones):
        ...
        try:
            ...
            log.warning(
                "Validation error (%s) for %s with %s", e, geozone["nom"], params
            )
            continue
    ...
    for dataset in Dataset.objects(spatial__zones__gt=[]):
        ...
        dataset.update(
            spatial=SpatialCoverage(
                granularity=dataset.spatial.granularity, zones=list(new_zones)
            )
        )
