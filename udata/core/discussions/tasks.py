...
def notify_new_discussion(discussion_id):
    ...
    if isinstance(discussion.subject, (Dataset, Reuse, Post)):
        ...
        subject = _(
            "Your %(type)s have a new discussion", type=discussion.subject.verbose_name
        )
        mail.send(
            subject,
            recipients,
...
def notify_new_discussion_comment(discussion_id, message=None):
    ...
    if isinstance(discussion.subject, (Dataset, Reuse, Post)):
        ...
        recipients = list(
            {u.id: u for u in recipients if u != message.posted_by}.values()
        )
        subject = _(
            "%(user)s commented your discussion", user=message.posted_by.fullname
        )
