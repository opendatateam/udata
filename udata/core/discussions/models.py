...
class Message(SpamMixin, db.EmbeddedDocument):
    ...
    def spam_report_message(self, breadcrumb):
        ...
        message += (
            f" sur la discussion « [{discussion.title}]({discussion.external_url}) »"
        )
        return message
...
class Discussion(SpamMixin, db.Document):
    ...
    def external_url(self):
        return self.subject.url_for(
            _anchor="discussion-{id}".format(id=self.id), _external=True
        )

    def spam_report_message(self, breadcrumb):
        message = (
            f"Spam potentiel sur la discussion « [{self.title}]({self.external_url}) »"
        )
        if self.user:
