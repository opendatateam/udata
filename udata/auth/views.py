...
def send_change_email_confirmation_instructions(user, new_email):
    ...
    confirmation_link = url_for(
        "security.confirm_change_email", token=token, _external=True
    )
...
def change_email():
    ...
    return _security.render_template(
        "security/change_email.html", change_email_form=form
    )
...
def create_security_blueprint(app, state, import_name):
    ...
    if state.passwordless:
        bp.route(
            app.config["SECURITY_LOGIN_URL"], methods=["GET", "POST"], endpoint="login"
        )(send_login)
        bp.route(
    ...
    if state.registerable:
        bp.route(
            app.config["SECURITY_REGISTER_URL"],
            methods=["GET", "POST"],
            endpoint="register",
        )(register)
    ...
    if state.changeable:
        bp.route(
            app.config["SECURITY_CHANGE_URL"],
            methods=["GET", "POST"],
