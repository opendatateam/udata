    ...
    def validate(self):
        ...
        if self.user.password_rotation_demanded:
            self.password.errors.append(
                _("Password must be changed for security reasons")
            )
            return False
...
class ChangeEmailForm(Form):
    new_email = fields.StringField(
        _("New email"), [validators.DataRequired(), validators.Email()]
    )
    new_email_confirm = fields.StringField(
        _("Retype email"),
        [
            validators.EqualTo("new_email", message=_("Email does not match")),
            validators.Email(),
        ],
    )
