...
<template>
<div class="image-picker-field" data-sizes="{{sizes}}" data-basename="{{field.name}}" data-endpoint="{{field.endpoint}}">
    <div class="image-picker-preview">
        <img v-el="preview" src="{{field.src || default_img}}" data-placeholder="{{default_img}}"/>
    </div>
    <span class="image-picker-btn btn btn-default btn-file"
        v-el="btn" v-on="click: click">
        {{ _('Choose a picture') }}
            <!-- field.filename(),
