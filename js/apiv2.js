...
const APIV2 = new SwaggerClient({
    debug: config.debug,
    url: config.apiv2_specs,
    useJQuery: true,
    success() {
 ...
 */
APIV2.resolve = function($ref) {
    const def = $ref.split('#')[1].replace('/definitions/', '');
    return this.definitions[def];
};

/**
 * Register a callback to be called when APIV2 is ready.
 * The callback should have the following signature: function(APIV2)
 * @param  {Function} callback A callback to call when APIV2 is ready.
 */
APIV2.onReady = function(callback) {
    if (this.ready && this.isBuilt) {
        // APIV2 is already ready, call the callback immediately
        callback(this);
    } else {
