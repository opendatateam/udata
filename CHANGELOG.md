...
- Fixing a recurrent bug - see logs in Sentry - when opening the drag'n'drop modal directly from a dataset public view for uploading resources (clicking on the `add a resource` button/block) or when trying to update a resource directly from the dataset's public view (clicking on the `update the resource` button) [#2483](https://github.com/opendatateam/udata/pull/2483)...
