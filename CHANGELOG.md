...
- Static assets are now compatible with long-term caching (ie. their hash is present in the filename). :warning: You may need to run `inv assets-build` to generate an initial `manifest.json`, both in `udata` and in your theme extension. See [#1826](https://github.com/opendatateam/udata/pull/1826) for full details.
